...
import UserDetailManager from "../user/UserDetailManager";
                ...
                {data.map(item => (
                    <div key={`item-${item.id}`}>
                        <div style={{ "background": "#cfcfcf" }}>
                            <span><Link to={`/users/${item.author.username}`}> Author: {item.author.profile.nickname}</Link></span>
